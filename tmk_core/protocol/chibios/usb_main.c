...
uint8_t es_usb_std_req_get_inf_value = 0;
...
static bool usb_requests_hook_cb(USBDriver *usbp) {
	...
	/*\lib\chibios\os\hal\src\hal_usb.c   static bool default_handler(USBDriver *usbp)*/
  ...
  case (uint32_t)USB_RTYPE_RECIPIENT_INTERFACE | ((uint32_t)USB_REQ_GET_INTERFACE << 8):
        usbSetupTransfer(usbp, &es_usb_std_req_get_inf_value, 1, NULL);
        return true;
